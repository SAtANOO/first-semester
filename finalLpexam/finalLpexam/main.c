...
////  finalLpexam
////
////  Created by Ashish Acharya on 21/01/2024.
//
//#include <stdio.h>
//#include<time.h>
//#include <string.h>
...
typedef struct{
 ...
 int get_details( detail *person, int *records){
     if (*records>=max){
         printf("errror the maximum detail reached\n");
         return -1;
     }
 printf("enter the name:");
   ...
   fgets(person->name,max_name,stdin);
    ...
    fseek(stdin,0,SEEK_END);
   fgets(person->address,max_add,stdin);
    printf("enter the age:");
     fseek(stdin,0,SEEK_END);
    scanf("%d", &person->age);
     printf("enter the key for identification:\n");
     scanf("%d",&person->key);

    return 0;
 }
 void print_detail(detail *person){
     printf("person detail");
     printf("name:%s",person->name);
     printf("adress:%s",person->address);
     printf("age:%d",person->age);
 }
 void save_detail(detail * person , int numrecords){
     FILE *fptr=fopen("detail.txt","wb");
     if(fptr==NULL){
         printf("Error opening file:");
         return;
         }
         fwrite(&numrecords,sizeof(int),1,fptr);
         fwrite(person, sizeof(detail),1,fptr);
         fclose(fptr);
         printf("event saved successfully...");
 }
 
 int load_details(detail * person , char *file){
    ...
    if(file !=NULL){
        int numrecords;
        fread(&numrecords,sizeof(int),1,ptr);
        if(numrecords>max){
            printf("Error maximum event reached..\n");
            return 0;
        }
    fseek(ptr,sizeof(int),SEEK_SET);
    fread(person,sizeof(detail),1,ptr);
    fclose(ptr);
    printf("event succesfully loaded from %s for reading",file);
 ...
 int menu(){
     printf("\n \t MENU.\n");
     printf("[1] insert the derails.\n");
     printf("[2] list information about details.\n");
     printf("[3] exit and save the details.\n");
     printf("[4] delete the detail.\n");
     int c;
     printf("enter the choice:");
     scanf("%d",&c);
     return c;
 }
detail *delete_detail( detail * name , detail *last){
    int key;
    printf("enter the key to remove the detail:");
    scanf("%d",&key);
    detail *ptr;
    for (ptr=name; ptr<last; ptr++) {
        if (ptr->key == key) {
            printf("removing details - [%4d] %s\n",ptr->key,ptr->name);
            for (detail *next = ptr+1; next<last; ptr++,next++) {
                *ptr=*next;
            }
            return last-1;
        }
    }
    printf("detail with the %d not found \n",key);
    return last;
    }
    

    int main(){
        detail names[max_name];
        int numrecords =load_details(names, "detail.txt");
        char name[50];
        int c;
        do{
            c=menu();
        switch(c){
            case 1:
            get_details(names,&numrecords);
            numrecords++;
            break;
            
            
            case 2:
            printf("enter the name to print the detail:");
            fseek(stdin,0,SEEK_END);
            fgets(name,50,stdin);
            print_detail(names);
            break;
            ...
            case 3:
            save_detail(names, numrecords);
            printf("number of the save events in the file %d",numrecords);
            return 1;
            default:
            printf("! invalid try again\n");
            break;
            case 4: {
                ...
                fseek(stdin, 0, SEEK_END);
                                scanf("%s", name);
                detail* last = delete_detail(name, name+ numrecords );
                                if (last != name + numrecords)
                                    numrecords--;
                break;
            }
